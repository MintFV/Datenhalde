...
class User(UserMixin, db.Model):  # type: ignore[name-defined]
    ...
    role = db.Column(db.String(20), nullable=False, default="user")  # 'admin' oder 'user'
...
class Tenant(db.Model):  # type: ignore[name-defined]
    ...
    id = db.Column(db.Integer, primary_key=True)
    tenant_id = db.Column(db.String(100), unique=True, nullable=False, index=True)
    ...
    mqtt_accounts = db.relationship(
        "MqttAccount", backref="tenant", lazy=True, cascade="all, delete-orphan"
    )
...
class Device(db.Model):  # type: ignore[name-defined]
    ...
    device_type = db.Column(db.String(20), nullable=False)  # tasmota / rpi / telegraf
    description = db.Column(db.String(200), nullable=True)
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC))

    mqtt_account_id = db.Column(db.Integer, db.ForeignKey("mqtt_accounts.id"), nullable=True)
    mqtt_account = db.relationship("MqttAccount", backref="device", uselist=False)
